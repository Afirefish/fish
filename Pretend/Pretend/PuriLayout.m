...
-(UICollectionViewLayoutAttributes *)layoutAttributesForHorozontalScrollForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CGFloat angle = arcOffset/_radius;//我觉得是这样的
    ...
    attributes.size = _itemSize;//设置了cell的大小
    int mirrorX = _mirrorX ? -1 : 1;//传入no值，初始化为1，控制正向旋转还是反向旋转
    int mirrorY = _mirrorY ? -1 : 1;

    CGFloat x = _centre.x + offset + mirrorX*(_radius*cosf(indexPath.item*angleOfEachItem - offsetAngle + angleOfEachItem/2 - _startAngle));//圆心变换，然后加上开始位置的角度与当前cell中心点角度的角度差计算出来的x值
